'use client';
import { Home, Star, Bell, TrendingUp, Wallet, Menu } from 'lucide-react';
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-[#0b0e17] border-r border-gray-800 z-40 transform transition lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex flex-col h-full">
          ...
          <nav className="space-y-1 text-sm flex-1">
            {menu.map(item => (
              <a key={item.label} href={item.href} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-900/50 transition">
